...
- Watch `nuthatch_rows_decoded` / `nuthatch_last_block` climb. On a TTY, `dev` shows a live progress
...
- Check `nuthatch_tip` vs `nuthatch_last_block` — the gap is the lag. `nuthatch_sealed_through` trails
...
- Reorgs only ever touch the **hot store** — sealed segments are immutable. You should never see sealed
  data change. `nuthatch_reorgs` counts detections; the hot store rolls back and IVM views retract
...
- The budget is per-runtime and CI-enforced. In a roost it's shared across nests (`max_rss_mb`, default
  2048); a mount projected to exceed it is refused. Check actual `rss_bytes` in the roster.
